...
class EPub {
	 ...
	 * @return bool $success
	 */
	function addChapter($chapterName, $fileName, $chapterData, $autoSplit = FALSE, $externalReferences = EPub::EXTERNAL_REF_IGNORE, $baseDir = "", $toToc = true) {
		if ($this->isFinalized) {
			return FALSE;
		}
		$fileName = preg_replace('#\\\#i', "/", $fileName);
	...
	function setCoverImage($fileName, $imageData = NULL, $mimetype = NULL) {
		...
		$this->opf_spine = "\t\t<itemref idref=\"coverPage\" linear=\"yes\" />\n" . $this->opf_spine;
